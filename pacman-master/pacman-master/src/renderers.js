...
var initRenderer = function(){
    ...
    ArcadeRenderer.prototype = newChildObject(CommonRenderer.prototype, {
        ...
        drawMap: function(isCutscene) {
            // Bắt đầu vẽ bản đồ, gọi hàm để thiết lập các yếu tố cơ bản
            beginMapFrame();
        
            if (map) {
                // Nếu bản đồ hiện có, kiểm tra trạng thái và tắt hiệu ứng flash nếu không phải ở trạng thái "finishState"
                if (state != finishState) {
                    this.flashLevel = false;
                }
        
                var x, y;   // Khai báo biến x, y để lặp qua các cột và hàng của bản đồ
                var i, j;
                var tile;    // Biến để chứa các tile (gạch) trên bản đồ
        
                // Vẽ cửa nhà ma
                i = 0;
                for (y = 0; y < map.numRows; y++) // Duyệt qua các hàng
                    for (x = 0; x < map.numCols; x++) { // Duyệt qua các cột
                        // Nếu gặp cặp dấu '-' liên tiếp trên bản đồ, vẽ cửa nhà ma
                        if (map.currentTiles[i] == '-' && map.currentTiles[i + 1] == '-') {
                            bgCtx.fillStyle = "#ffb8de";  // Chọn màu hồng cho cửa
                            bgCtx.fillRect(x * tileSize, y * tileSize + tileSize - 2, tileSize * 2, 2); // Vẽ hình chữ nhật cho cửa
                        }
                        i++; // Tiến đến tile tiếp theo
                    }
        
                // Kiểm tra xem có bật hiệu ứng flash hay không
                if (this.flashLevel) {
                    bgCtx.fillStyle = "#000";  // Màu nền đen
                    bgCtx.strokeStyle = "#fff";  // Đường viền màu trắng
                } else {
                    bgCtx.fillStyle = map.wallFillColor;  // Màu nền của tường
                    bgCtx.strokeStyle = map.wallStrokeColor;  // Màu đường viền của tường
                }
        
                // Vẽ các con đường trong bản đồ
                for (i = 0; i < map.paths.length; i++) {
                    var path = map.paths[i];
                    bgCtx.beginPath();
                    bgCtx.moveTo(path[0].x, path[0].y);  // Di chuyển đến điểm đầu tiên trong con đường
        
                    // Duyệt qua các điểm tiếp theo trong con đường và vẽ các đoạn thẳng hoặc đường cong
                    for (j = 1; j < path.length; j++) {
                        if (path[j].cx != undefined) {
                            bgCtx.quadraticCurveTo(path[j].cx, path[j].cy, path[j].x, path[j].y);  // Vẽ đường cong bậc 2
                        } else {
                            bgCtx.lineTo(path[j].x, path[j].y);  // Vẽ đoạn thẳng
                        }
                    }
        
                    // Khép kín đường (tạo hình vòng tròn khép kín)
                    bgCtx.quadraticCurveTo(path[j - 1].x, path[0].y, path[0].x, path[0].y);
                    bgCtx.fill();  // Tô màu cho vùng bên trong
                    bgCtx.stroke();  // Vẽ đường viền
                }
        
                // Vẽ các viên đạn trên bản đồ
                bgCtx.fillStyle = map.pelletColor;  // Màu viên đạn
                i = 0;
                for (y = 0; y < map.numRows; y++) // Duyệt qua các hàng
                    for (x = 0; x < map.numCols; x++) { // Duyệt qua các cột
                        this.refreshPellet(x, y, true);  // Gọi hàm để làm mới các viên đạn
                    }
        
                // Nếu có hàm onDraw được định nghĩa trong bản đồ, gọi nó
                if (map.onDraw) {
                    map.onDraw(bgCtx);
                }
        
                // Nếu chỉ cần vẽ bản đồ mà không vẽ các yếu tố khác, kết thúc ở đây
            ...
            if (level > 0) {
                var numRows = 36;  // Số hàng trong bản đồ
                var numCols = 28;  // Số cột trong bản đồ
        
                if (!isCutscene) {
                    // Vẽ các mạng sống của người chơi
                    var i;
                    bgCtx.fillStyle = pacman.color;  // Màu của nhân vật Pac-Man
        
                    bgCtx.save();
                    bgCtx.translate(3 * tileSize, (numRows - 1) * tileSize);  // Di chuyển đến vị trí để vẽ mạng sống
                    bgCtx.scale(0.85, 0.85);  // Tỉ lệ thu nhỏ các mạng sống
        
                    var lives = extraLives == Infinity ? 1 : extraLives;  // Nếu số mạng sống là vô hạn, đặt mạng sống bằng 1
        
                    // Vẽ mạng sống tùy thuộc vào chế độ trò chơi
                    if (gameMode == GAME_PACMAN) {
                        for (i = 0; i < lives; i++) {
                            drawPacmanSprite(bgCtx, 0, 0, DIR_LEFT, Math.PI / 6);  // Vẽ sprite Pac-Man
                            bgCtx.translate(2 * tileSize, 0);  // Di chuyển để vẽ mạng sống tiếp theo
                        }
                    } else if (gameMode == GAME_MSPACMAN) {
                        for (i = 0; i < lives; i++) {
                            drawMsPacmanSprite(bgCtx, 0, 0, DIR_RIGHT, 1);  // Vẽ sprite Ms. Pac-Man
                            bgCtx.translate(2 * tileSize, 0);  // Di chuyển để vẽ mạng sống tiếp theo
                        }
                    } else if (gameMode == GAME_COOKIE) {
                        for (i = 0; i < lives; i++) {
                            drawCookiemanSprite(bgCtx, 0, 0, DIR_RIGHT, 1, false);  // Vẽ sprite Cookieman
                            bgCtx.translate(2 * tileSize, 0);  // Di chuyển để vẽ mạng sống tiếp theo
                        }
                    } else if (gameMode == GAME_OTTO) {
                        for (i = 0; i < lives; i++) {
                            drawOttoSprite(bgCtx, 0, 0, DIR_RIGHT, 0);  // Vẽ sprite Otto
                            bgCtx.translate(2 * tileSize, 0);  // Di chuyển để vẽ mạng sống tiếp theo
                        }
                    }
        
                    // Nếu mạng sống là vô hạn, vẽ biểu tượng vô cực
                    if (extraLives == Infinity) {
                        bgCtx.translate(-4 * tileSize, 0);  // Di chuyển lại
        
                        // Vẽ biểu tượng vô cực
                        var r = 2;  // Bán kính của mỗi nửa vòng
                        var d = 3;  // Khoảng cách giữa hai điểm tiêu cự
                        bgCtx.beginPath();
                        bgCtx.moveTo(-d - r, 0);
                        bgCtx.quadraticCurveTo(-d - r, -r, -d, -r);  // Vẽ phần trên của vô cực
                        bgCtx.bezierCurveTo(-(d - r), -r, d - r, r, d, r);  // Vẽ phần dưới của vô cực
                        bgCtx.quadraticCurveTo(d + r, r, d + r, 0);  // Vẽ phần cuối của vô cực
                        bgCtx.quadraticCurveTo(d + r, -r, d, -r);
                        bgCtx.bezierCurveTo(d - r, -r, -(d - r), r, -d, r);
                        bgCtx.quadraticCurveTo(-d - r, r, -d - r, 0);
                        bgCtx.lineWidth = 1;
                        bgCtx.strokeStyle = "#FFF";
                        bgCtx.stroke();  // Vẽ đường viền của biểu tượng vô cực
                    }
        
                    bgCtx.restore();  // Khôi phục lại bối cảnh ban đầu
                }
        
                // Vẽ trái cây (fruit) trong cấp độ hiện tại
                var fruits = fruit.fruitHistory;  // Lịch sử các trái cây đã xuất hiện
                var i, j;
                var f, drawFunc;
                var numFruit = 7;
                var startLevel = Math.max(numFruit, level);  // Đảm bảo trái cây được vẽ đúng theo cấp độ
        
                if (gameMode != GAME_PACMAN) {
                    startLevel = Math.min(numFruit, startLevel);  // Chỉ vẽ 7 trái cây đối với Ms. Pac-Man và các chế độ khác
                }
        
                var scale = 0.85;  // Tỉ lệ thu nhỏ trái cây
                for (i = 0, j = startLevel - numFruit + 1; i < numFruit && j <= level; j++, i++) {
                    f = fruits[j];
                    if (f) {
                        drawFunc = getSpriteFuncFromFruitName(f.name);  // Lấy hàm vẽ sprite trái cây
                        if (drawFunc) {
                            bgCtx.save();
                            bgCtx.translate((numCols - 3) * tileSize - i * 16 * scale, (numRows - 1) * tileSize);  // Vị trí vẽ trái cây
                            bgCtx.scale(scale, scale);  // Tỉ lệ thu nhỏ
                            drawFunc(bgCtx, 0, 0);  // Vẽ trái cây
                            bgCtx.restore();  // Khôi phục bối cảnh ban đầu
                        }
                    }
                }
                ...
                if (!isCutscene) {
                    if (level >= 100) {
                        bgCtx.font = (tileSize - 3) + "px ArcadeR";  // Kích thước font lớn hơn nếu cấp độ >= 100
                    } else {
                        bgCtx.font = (tileSize - 1) + "px ArcadeR";  // Kích thước font bình thường
                    }
        
                    bgCtx.textBaseline = "middle";
                    bgCtx.fillStyle = "#777";  // Màu chữ
                    bgCtx.textAlign = "left";
                    bgCtx.fillText(level, (numCols - 2) * tileSize, (numRows - 1) * tileSize);  // Vẽ cấp độ ở góc dưới bên trái
                }
            }
        
            // Kết thúc việc vẽ bản đồ
            endMapFrame();
        },
